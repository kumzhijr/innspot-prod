...
const BookRoomClient = () => {
    ...
    if(pageLoaded && !paymentSuccess && (!bookingRoomData || !clientSecret)) return <div className="flex items-center flex-col gap-4">
        <div className="text-rose-500">Oops! This page could not be loaded. Please try again later.</div>
        <div className="flex items-center gap-4">
        <Button variant='outline' onClick={()=> router.push('/')}>Go Home</Button>
        <Button onClick={()=> router.push('/my-bookings')}>My Bookings</Button>
        </div>
    </div>

    if(paymentSuccess) return <div className="flex items-center flex-col gap-4">
        <div className="text-teal-500 text-center">Payment Succesful</div>
        <Button onClick={()=> router.push('/my-bookings')}>My Bookings</Button>
        </div>
    ...
    return (<div className="max-w-[700px] mx-auto">
        ...
        {clientSecret && bookingRoomData && <div>
                <h3 className="text-4xl mb-6">Confirm and pay</h3>
                <div>
                    <div className="mb-6">
